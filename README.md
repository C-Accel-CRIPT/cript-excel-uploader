...
<img src="./docs_assets/screenshot_of_where_to_find_excel_uploader.png" 
    style="height: 15rem; weidth: 15rem;"
...
<ol id="usage-steps">
  ...
  <li>
    Create a <b>Group</b> inside of the <a href="https://www.criptapp.org/">CRIPT platform</a>
    <ul>
      <li>
  ...
  <li>
    Create a <b>Collection</b> inside the <a href="https://www.criptapp.org/">CRIPT platform</a>
    <ul>
      <li>
...
- **Windows**
  ...
  - Download <code>cript_uploader_vX-X-X.exe</code> from the [latest release](https://github.com/C-Accel-CRIPT/cript-excel-uploader/releases)
...
- **Mac or Linux**
  - Download <code>cript_uploader_vX-X-X</code> from the [latest release](https://github.com/C-Accel-CRIPT/cript-excel-uploader/releases)
  - Open a terminal and navigate the relevant folder
    ```bash
    cd <path_to_folder>
    ```
...
<h1>Customizing the <span style="color: var(--excel-light-color)">Excel</span> File</h1>
...
> `Relation` is a bit tricky to explain, so we dedicated a section that <a href="#relation-explanation">explains row 1 abstract category of `relation`</a>
...
- <span class="row-3">Row 3</span>: Defines the units for that column
  ...
  - `celsius`, `g/ml`
  - All the <a href="https://github.com/hgrecco/pint/blob/master/pint/default_en.txt">supported units are documented</a>
...
<table>
  ...
  <tr>
    <td>
      <span class="row-2">names</span>
    </td>
  </tr>
    <tr>
    <th>
      <span class="row-3">units</span>
    </th>
  </tr>
  <tr>
    <td class="row-4-required-optional-label">
    <span class="">
      <b>vinylbenzene; phenylethylene; ethenylbenzene; styrene</b>
    </span>
    </td>
  </tr>
</table>
...
<h2>The general structure for all sheets:</h2>
...
The row 1 field <code>relation</code> is essentially a way for a row from one sheet to reference another row in a different sheet.

In computer science terms, we could think of it as a one-to-one relationship from the <code>relation</code> row of one sheet to the <code>\*name</code> row of another sheet, the name field works well as an identifier because we are requiring all names to be unique.
...
Below there is a section that has a list of <a href="#individual-sheets-reference">valid values for each sheet's row 2</a>
...
- We can have more than one field present on a column header if needed
  ...
  - We can also show the `color` of the material at that `density` at that `temperature` with `density:temperature:color` and we can add on more
  - Examples
...
If there are multiple temperature throughout different time and we want to show each of their color (or any other property) we can use an `Id` field to differentiate between the different temperatures. We denote an `Id` with brackets and a number inside such as [1]. The `Id` is used to identify distinct properties/conditions of the same type
...
Here, we are recording the <span class="row-2">[1]density</span>, and then we are recording the temperature of that density with <span class="row-2">[1]density:temperature</span> and then we have a different <span class="row-2">[2]density</span> at a different time and then we are recording the temperature of that density <span class="row-2">[2]density:temperature</span>

<br>

<h1 id="individual-sheets-reference">
  Individual Sheets
</h1>

<h2 class="individual-sheet-explanation"> 
...
<em><b>Example</b> of a column within materials sheet that shows an example valid options:</em>
...
<h2> 
  <span class="optional-excel-sheet-color" style="text-decoration: underline">mixture component</span> 
  sheet 
...
The <span>mixture</span> sheet is simple. Row 1 is always <code>relation</code> and Row 2 is always
...
| Key | Key Type | Required | Expected Value |
| --- | --- | --- | --- |
| *experiment | relation | yes | string from `*name`column of`experiment` sheet | | *name | attribute | yes | unique string | unique | | *type | attribute | yes | any [process type](https://criptapp.org/keys/process-type/) | keywords | attribute | no | list of [keywords](https://criptapp.org/keys/process-keyword/) (e.g,`str1; str2; str3`) | description | attribute | no | string | | equipment | attribute | no | list of [equipment](https://criptapp.org/keys/equipment/) (e.g, `str1; str2; str3`) |
...
<h2>
  <span class="required-excel-sheet-color"><u>process ingredient</u></span> 
  sheet
</h2>
...
| Key | Key Type | Required | Value Type
| --- | --- | --- | --- |
| *process | relation | yes | string from `*name`column of`process`sheet | | *material | relation | yes | string from`*name`column of`material` sheet |
...
<h2>
  <span class="required-excel-sheet-color"><u>data</u></span> 
  sheet
</h2>
...
| Key | Key Type | Required | Value Type
| --- | --- | --- | --- |
| *experiment | relation | yes | string from `*name`column of`experiment`sheet | | *name | attribute | yes | unique string | unique | | *type | attribute | yes | any [data type](https://criptapp.org/keys/data-type/) | sample_prep | attribute | no | string | citation | relation | no | string from`\*name`column of`citation` sheet
| notes | attribute | no | string | |
...
<h2>
  <span class="required-excel-sheet-color"><u>citation</u></span> 
  sheet
</h2>
...
<blockquote>
  <code>Row 1:</code> can only be an <em>"attribute"</em> <br>
...
- **_What units can I use?_**
  ...
  Here is a list of the <a href="https://github.com/hgrecco/pint/blob/master/pint/default_en.txt">supported units</a> from the Pint Python package
